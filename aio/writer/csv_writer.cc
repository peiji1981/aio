...
CsvWriter::CsvWriter(WriterOptions op, std::unique_ptr<OutputStream> out,
    ...
    : options_(std::move(op)),
      ...
      inited_(false),
      out_(std::move(out)) {
  AIO_ENFORCE(options_.file_schema.feature_names.size());
  AIO_ENFORCE(options_.file_schema.feature_names.size() ==
              options_.file_schema.feature_types.size());
  AIO_ENFORCE(out_->Tellp() == 0);
  AIO_ENFORCE(options_.float_precision > 0 &&
...
bool CsvWriter::Add(const ColumnVectorBatch& data) {
  AIO_ENFORCE(inited_, "Please Call Init before use writer");
  const size_t rows = data.Shape().first;
  const size_t cols = data.Shape().second;
  AIO_ENFORCE(cols == options_.file_schema.feature_names.size());
  ...
  for (size_t r = 0; r < rows; r++) {
    for (size_t c = 0; c < cols; c++) {
      switch (types[c]) {
        ...
        default:
          AIO_THROW("unknow Schema::type {}", types[c]);
      }
      if (c + 1 != cols) {

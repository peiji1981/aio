...
#include "aio/base/float.h"
#include "aio/stream/file_io.h"
#include "fmt/format.h"

#include "aio/reader/mmapped_file.h"
...
void CsvReader::Init() {
  AIO_ENFORCE(!inited_, "DO NOT call init multiply times");

  ParseHeader();

  const auto& schema = options_.file_schema;
  AIO_ENFORCE(schema.feature_names.size() == schema.feature_types.size());
  const size_t f_size = schema.feature_names.size();
  // col reader do not support empty feature.
  AIO_ENFORCE(options_.column_reader == false || f_size > 0);
  ...
  for (size_t i = 0; i < f_size; i++) {
    ...
    selected_features_.push_back({idx, schema.feature_types[i]});
  }
...
void CsvReader::ParseHeader() {
  ...
  AIO_ENFORCE(NextLine(&headers), "Can't get header from file '{}'",
              in_->GetName());
...
void CsvReader::UpdateRowMap() {
  ...
  AIO_ENFORCE(!options_.column_reader);
...
void CsvReader::BuildMmapFiles() {
  ...
  while (NextLine(&fields)) {
    ...
    for (size_t i = 0; i < selected_features_.size(); i++) {
      ...
      switch (type) {
        ...
        default:
          AIO_THROW("unknow Schema::type {}", type);
      }
    }
  }
  total_rows_ = row_count;
  {
    ...
    std::optional<aio::Exception> ope;
    for (auto& os : oss) {
      try {
        os->Close();
      } catch (const aio::Exception& e) {
        ope = e;
      }
    }
...
bool CsvReader::Next(ColumnVectorBatch* data) {
  AIO_ENFORCE(inited_, "Please Call Init before use reader");
  data->Clear();
  if (options_.column_reader) {
    return NextCol(data);
  } else {
...
bool CsvReader::NextCol(ColumnVectorBatch* data) {
  AIO_ENFORCE(total_rows_ != kUnknowTotalRow);
  ...
  switch (type) {
    case Schema::STRING: {
      ...
      size_t pos = 0;
      while (pos < mmap_size) {
        AIO_ENFORCE(pos + sizeof(uint32_t) <= mmap_size);
        // may not aligned. works fine on X86.
        uint32_t len = *reinterpret_cast<const uint32_t*>(mmap_data + pos);
        pos += sizeof(uint32_t);

        AIO_ENFORCE(pos + len <= mmap_size);
        const char* str = mmap_data + pos;
        pos += len;

        col.emplace_back(str, len);
      }
      AIO_ENFORCE(col.size() == total_rows_);
    ...
    case Schema::FLOAT: {
      ...
      col.reserve(total_rows_);
      AIO_ENFORCE(mmap_size == total_rows_ * sizeof(float));
    ...
    case Schema::DOUBLE: {
      ...
      col.reserve(total_rows_);
      AIO_ENFORCE(mmap_size == total_rows_ * sizeof(double));
...
void CsvReader::Seek(size_t index) {
  AIO_ENFORCE(inited_, "Please Call Init before use reader");
  if (options_.column_reader) {
    AIO_ENFORCE(index < selected_features_.size(),
                "seek for col out of range, try {} max {}", index,
                selected_features_.size());
    current_index_ = index;
  } else {
    AIO_ENFORCE(total_rows_ == kUnknowTotalRow || index < total_rows_,
                "seek for row out of range, try {} max {}", index, total_rows_);
    auto it = rows_map_.upper_bound(index);
    AIO_ENFORCE(it != rows_map_.begin());
    std::advance(it, -1);
    in_->Seekg(it->second);
    current_index_ = it->first;
    while (current_index_ < index && NextLine(NULL)) {
      current_index_++;
    }
    AIO_ENFORCE(current_index_ == index,
...
std::unique_ptr<Reader> CsvReader::Spawn() {
  AIO_ENFORCE(inited_, "CAN NOT Spawn before init");
  auto in = in_->Spawn();
  AIO_ENFORCE(in_->Tellg() == in->Tellg());

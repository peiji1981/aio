...
#include "aio/base/elapsed_timer.h"
#include "aio/base/exception.h"
#include "aio/base/schema.h"
...
TEST(RW, test) {
  // in this case, we steaming read from input & write into other file.
  {
    ...
    std::unique_ptr<InputStream> in(
        new FileInputStream("aio/test/data/perfect_logit_a.csv"));
    ...
    while (reader->Next(&batch_dataset)) {
      batch_row_count += batch_dataset.Shape().first;
    ...
    EXPECT_EQ(batch_dataset.Shape().first, 509);
    EXPECT_EQ(batch_dataset.Shape().second, 12);
    ...
    EXPECT_EQ(batch_dataset.Shape().first, 701);
    EXPECT_EQ(batch_dataset.Shape().second, 12);
  }
  // now let try read csv file by col.
  {
    ...
    EXPECT_EQ(rows2, batch_dataset.Shape().first);
    EXPECT_EQ(1, batch_dataset.Shape().second);
    // id1 col.
    EXPECT_EQ(reader->Next(&batch_dataset), true);
    EXPECT_EQ(rows2, batch_dataset.Shape().first);
    EXPECT_EQ(1, batch_dataset.Shape().second);
    ...
    EXPECT_EQ(rows2, batch_dataset.Shape().first);
    EXPECT_EQ(12, batch_dataset.Shape().second);
    ...
    EXPECT_EQ(spawn_reader->Next(12, &batch_dataset), true);
    EXPECT_EQ(rows2, batch_dataset.Shape().first);
    EXPECT_EQ(2, batch_dataset.Shape().second);
    ...
    EXPECT_EQ(rows2, batch_dataset.Shape().first);
    EXPECT_EQ(5, batch_dataset.Shape().second);
  }
}
...
TEST(RW, empty_reader) {
  {
    ...
    while (reader->Next(&batch_dataset)) {
      batch_row_count += batch_dataset.Shape().first;
      EXPECT_EQ(0, batch_dataset.Shape().second);
    }
    // reach EOF
    EXPECT_EQ(batch_row_count, 10000);
  }
  {
    ...
    auto reader = std::make_shared<CsvReader>(read_options, std::move(in));
    EXPECT_THROW(reader->Init(), aio::Exception);
  }
}
...
TEST(LARGE, test) {
  ...
  EXPECT_EQ(FLAGS_rows, batch_dataset_col.Shape().first);
  EXPECT_EQ(1, batch_dataset_col.Shape().second);
  ...
  while (test_count--) {
    ...
    EXPECT_LT(0, batch_dataset_row.Shape().first);

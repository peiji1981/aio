...
u3, 1e-300 , 0 , 0 , -inf , inf
  ...
  {  // reader
    ...
    EXPECT_EQ(reader.Rows(), std::numeric_limits<size_t>::max());
    EXPECT_EQ(batch.Shape().first, 1);
    EXPECT_EQ(batch.Shape().second, 6);
  ...
  }
  {
    ...
    {  // reader
      ...
      EXPECT_EQ(reader.Next(&batch), true);
      EXPECT_EQ(batch.Shape().first, 2);
      EXPECT_EQ(batch.Shape().second, 3);
    }
    ...
    {  // reader
      ...
      EXPECT_EQ(reader.Next(&batch), true);
      EXPECT_EQ(batch.Shape().first, 2);
      EXPECT_EQ(batch.Shape().second, 4);
...
u2, 22 , 23 , 24 , 25 , 0
  ...
  {  // reader
    ...
    EXPECT_EQ(reader.Tell(), 0);
    EXPECT_THROW(reader.Tellg(), aio::Exception);

    ColumnVectorBatch batch;
    EXPECT_EQ(reader.Next(&batch), true);
    EXPECT_EQ(reader.Tell(), 1);
    EXPECT_EQ(batch.Shape().first, 2);
    EXPECT_EQ(batch.Shape().second, 1);
...
u2, 22 , 23 , 24 , 25 , 0
    ...
    EXPECT_THROW(reader.Next(&data), aio::Exception);
  }
